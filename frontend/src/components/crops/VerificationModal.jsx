        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
                <div className="px-6 py-4 border-b flex items-center justify-between bg-gray-50">
                    <div>
                        <h2 className="text-xl font-bold text-gray-900">Crop Inspection</h2>
                        <p className="text-sm text-gray-500">Verifying: {crop.cropName} by Farmer ID: {crop.farmerId?.name || crop.farmerId}</p>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-6">
                    <div className="flex gap-4">
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 p-4 rounded-xl border-2 transition-all ${status === 'Verified' ? 'border-green-600 bg-green-50 text-green-700' : 'border-gray-200 text-gray-500 hover:border-green-200'}`}
                        >
                        ...
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 p-4 rounded-xl border-2 transition-all ${status === 'Rejected' ? 'border-red-600 bg-red-50 text-red-700' : 'border-gray-200 text-gray-500 hover:border-red-200'}`}
                        >
                    ...
                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-2">Verification Report</label>
                        <textarea
                            required
                    ...
                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-2">Inspection Images (Optional)</label>
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                            ...
                            <label className="aspect-square flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-primary-500 transition-colors">
                                <Upload className="h-6 w-6 text-gray-400" />
                                <span className="text-xs text-gray-500 mt-1">Add Photo</span>
                                <input type="file" className="sr-only" multiple onChange={handleImageUpload} accept="image/*" />
                            </label>
                        </div>
                        {uploading && <p className="text-xs text-primary-600 mt-2 flex items-center animate-pulse"><Loader2 className="h-3 w-3 animate-spin mr-1" /> Uploading...</p>}
                    </div>
